...
impl ModelWeights {
    ...
    fn prepare_attention_mask(
        ...
        let mask = Tensor::cat(&mask, 0)?.contiguous()?;
        let on_true = mask.zeros_like()?.to_dtype(DType::U8)?;
        #[allow(clippy::useless_conversion)]
        let on_false = Tensor::new(u8::from(1), device)?
    ...
    pub fn forward(
        ...
        let mask: Option<Tensor> = match attn_mask {
            Some(mask) => Some(self.prepare_attention_mask(seq_len, mask, x.device())?),
            None => {
                if seq_len <= 1 {
                    None
                } else {
