...
pub fn apps_interface_view() -> Element {
    ...
    let diagram_code: Memo<(String, String)> = use_memo(move || {
        ...
        (diagram_code, builder_error.unwrap_or_default())
    });
...
#[component]
pub fn mermaid_view(
    diagram_code: Memo<(String, String)>,
) -> Element {
    ...
    let _ = use_resource(move || async move {
        ...
        eval.send(diagram_code().1).unwrap();
