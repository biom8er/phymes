...
pub static MERMAID_FLOWCHART_NODES_TEMPLATE: &str = r#"{%- for row in rows %}
            {{ row.node_name }}@{ shape: {{ row.node_shape }}, label: '{{ row.node_label }}' }
{%- endfor %}"#;

/// Mermaid.js flowchart links section jinja2 template
pub static MERMAID_FLOWCHART_LINKS_TEMPLATE: &str = r#"
{%- for row in rows %}
    {%- if row.link_text %}
...
///     .join("");
/// ```
pub static MERMAID_FLOWCHART_INPUT: &str = r#"{
"direction": "{{ direction }}"
}"#;
