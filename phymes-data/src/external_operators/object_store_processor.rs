    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match &mut self.state {
            ObjectStoreState::NotStarted => {
                ...
                self.state = ObjectStoreState::Connecting(Box::pin(fut));
                self.poll_next(cx)
            }
                ...
                Ok(response) => {
                    ...
                    match self.config.as_ref().unwrap().request_schema {
                        HTTPClientRequestSchemas::Messages => {
                            ...
                            self.state = ObjectStoreState::ToText(Box::pin(text));
                            self.poll_next(cx)
                        }
                        HTTPClientRequestSchemas::Attachments => {
                            let bytes = response.bytes();
                            self.state = ObjectStoreState::ToBytes(Box::pin(bytes));
                            self.poll_next(cx)
                        }
                        _ => {
                            let bytes = response.bytes();
            ...
            ObjectStoreState::ToText(fut) => match ready!(fut.as_mut().poll_unpin(cx)) {
                Ok(text) => {
                    ...
                    self.state = ObjectStoreState::Ready(vec![batch]);
                    self.poll_next(cx)
                }
                Err(err) => {
                    self.state = ObjectStoreState::Done;
            ...
            ObjectStoreState::ToBytes(fut) => match ready!(fut.as_mut().poll_unpin(cx)) {
                Ok(bytes) => {
                    // Initialize the metrics
                    let baseline_metrics =
                        if let Some(diagnostic_builder) = &self.diagnostic_builder {
            ...
            ObjectStoreState::Ready(batches) => {
                // Ready the next poll
                            ...
                            Some(
                                diagnostic_builder
                                    ...
                                    .baseline_metrics(
                                        line!(),
                                        file!(),
                                        "poll_next.ObjectStoreState::Ready",
                                    ),
                            )
                        } else {
                            None
