use std::{
    collections::VecDeque, io::Write, pin::Pin, sync::Arc, task::{Context, Poll, ready},
};
...
use object_store::{GetResult, MultipartUpload, ObjectMeta, ObjectStore, ObjectStoreExt, PutPayload, PutResult, path::Path};
...
use crate::{
    DataConfigTrait, ObjectStoreConfig, ObjectStoreOptsType,
};
...
impl Stream for ObjectStoreStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match &mut self.state {
            ...
            ObjectStoreState::StorageReaderStreamResult(fut) => match ready!(fut.as_mut().poll_next_unpin(cx)) {
                Some(Ok(bytes)) => {
                    // Initialize the metrics
                    let baseline_metrics =
                        if let Some(diagnostic_builder) = &self.diagnostic_builder {
            ...
            ObjectStoreState::StorageWriterMultipart(fut) => match ready!(fut.as_mut().poll_unpin(cx)) {
                Ok(mp) => {
                    ...
                    let chunk_size = self.config.as_ref().unwrap().chunk_size.ok_or(anyhow!("Missing `chunk_size` for multipart put in ObjectStoreStream."))?;
            ...
            ObjectStoreState::StorageWriterComplete(fut) => match ready!(fut.as_mut().poll_unpin(cx)) {
                Ok(results) => {
                    // Initialize the metrics
                    let baseline_metrics =
                        if let Some(diagnostic_builder) = &self.diagnostic_builder {
