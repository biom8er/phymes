...
impl DataOperatorTrait for ExtractPDFText {
    ...
    fn get_schema_output(
        ...
        let chunk_id = Field::new("chunk_id".to_string(), DataType::Utf8, false);
        let document_id = Field::new(self.lhs_pk.clone(), DataType::Utf8, false);
        let text = Field::new("text".to_string(), DataType::Utf8, false);
        let mut fields = vec![chunk_id, document_id, text];
        if let Some(other) = other {
            fields.extend(other);
        }
        Some(Arc::new(Schema::new(fields)))
    ...
    fn check_schema_lhs_input(&self, other: arrow::datatypes::SchemaRef) -> Result<Option<bool>> {
        ...
        if other.column_with_name(&self.lhs_values).is_none() {
            return Err(anyhow!(
                "LHS input is missing column for lhs_values {}.",
                self.lhs_values
            ));
        }
        Ok(Some(true))
