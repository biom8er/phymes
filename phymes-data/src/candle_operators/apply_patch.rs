...
pub fn apply_patch(
    ...
    let rhs_columns = rhs_args.first().ok_or(anyhow!("Missing rhs_args in apply_patch."))?.schema().fields().iter().map(|f| f.name().to_string()).collect::<Vec<_>>();
    let lhs_columns = lhs_args.first().ok_or(anyhow!("Missing lhs_args in apply_patch."))?.schema().fields().iter().map(|f| f.name().to_string()).collect::<Vec<_>>();

    // Filter RHS for `Delete`
    let rhs_delete = {
        let select_lhs_cols = rhs_columns.iter().map(|f| f.as_str()).chain(["delete"]).collect::<Vec<_>>();
        let select_rhs_cols = select_lhs_cols.iter().map(|_| "").collect::<Vec<_>>();
        let select_column_operators = rhs_columns.iter().map(|_| DataColumnOperator::None).chain([DataColumnOperator::Value]).collect::<Vec<_>>();
        let select_cast_operators = rhs_columns.iter().map(|_| DataCastOperator::None).chain([DataCastOperator::None]).collect::<Vec<_>>();
        let select_cast_datatypes = rhs_columns.iter().map(|_| DataType::Utf8).chain([DataType::Utf8]).collect::<Vec<_>>();
        let patch_operator = PatchOperator::Delete.to_string();
        let select_cast_templates = rhs_columns.iter().map(|_| "").chain([patch_operator.as_str()]).collect::<Vec<_>>();
        let rhs_delete = select(&select_lhs_cols, rhs_args, &select_rhs_cols, &select_lhs_cols, &select_lhs_cols, &select_column_operators, &select_cast_operators, &select_cast_datatypes, &select_cast_templates, device)?;
        filter(&[operator_column], &[rhs_delete], &["delete"], &[DataComparatorOperator::Like], &DataComparatorPredicate::All, device)?
    };
    ...
    let lhs_delete = {
        ...
        let select_rhs_cols = select_lhs_cols.iter().map(|_| "").collect::<Vec<_>>();
        let select_column_operators = lhs_columns.iter().map(|_| DataColumnOperator::None).collect::<Vec<_>>();
        let select_cast_operators = lhs_columns.iter().map(|_| DataCastOperator::None).collect::<Vec<_>>();
        let select_cast_datatypes = lhs_columns.iter().map(|_| DataType::Utf8).collect::<Vec<_>>();
        let select_cast_templates = lhs_columns.iter().map(|_| "").collect::<Vec<_>>();
        select(&select_lhs_cols, &[lhs_deleted], &select_rhs_cols, &select_lhs_cols, &select_lhs_cols, &select_column_operators, &select_cast_operators, &select_cast_datatypes, &select_cast_templates, device)?
    };
    

    // Filter RHS for `Update`
    let rhs_update = {
        let select_lhs_cols = rhs_columns.iter().map(|f| f.as_str()).chain(["update"]).collect::<Vec<_>>();
        let select_rhs_cols = select_lhs_cols.iter().map(|_| "").collect::<Vec<_>>();
        let select_column_operators = rhs_columns.iter().map(|_| DataColumnOperator::None).chain([DataColumnOperator::Value]).collect::<Vec<_>>();
        let select_cast_operators = rhs_columns.iter().map(|_| DataCastOperator::None).chain([DataCastOperator::None]).collect::<Vec<_>>();
        let select_cast_datatypes = rhs_columns.iter().map(|_| DataType::Utf8).chain([DataType::Utf8]).collect::<Vec<_>>();
        ...
        let select_cast_templates = rhs_columns.iter().map(|_| "").chain([patch_operator.as_str()]).collect::<Vec<_>>();
        let rhs_update = select(&select_lhs_cols, rhs_args, &select_rhs_cols, &select_lhs_cols, &select_lhs_cols, &select_column_operators, &select_cast_operators, &select_cast_datatypes, &select_cast_templates, device)?;
        ...
        let select_rhs_cols = select_lhs_cols.iter().map(|_| "").collect::<Vec<_>>();
        let select_column_operators = rhs_columns.iter().map(|_| DataColumnOperator::None).collect::<Vec<_>>();
        let select_cast_operators = rhs_columns.iter().map(|_| DataCastOperator::None).collect::<Vec<_>>();
        let select_cast_datatypes = rhs_columns.iter().map(|_| DataType::Utf8).collect::<Vec<_>>();
        let select_cast_templates = rhs_columns.iter().map(|_| "").collect::<Vec<_>>();
        select(&select_lhs_cols, &[rhs_update], &select_rhs_cols, &select_lhs_cols, &select_lhs_cols, &select_column_operators, &select_cast_operators, &select_cast_datatypes, &select_cast_templates, device)?
    };
    ...
    let rhs_create = {
        let select_lhs_cols = rhs_columns.iter().map(|f| f.as_str()).chain(["create"]).collect::<Vec<_>>();
        let select_rhs_cols = select_lhs_cols.iter().map(|_| "").collect::<Vec<_>>();
        let select_column_operators = rhs_columns.iter().map(|_| DataColumnOperator::None).chain([DataColumnOperator::Value]).collect::<Vec<_>>();
        let select_cast_operators = rhs_columns.iter().map(|_| DataCastOperator::None).chain([DataCastOperator::None]).collect::<Vec<_>>();
        let select_cast_datatypes = rhs_columns.iter().map(|_| DataType::Utf8).chain([DataType::Utf8]).collect::<Vec<_>>();
        ...
        let select_cast_templates = rhs_columns.iter().map(|_| "").chain([patch_operator.as_str()]).collect::<Vec<_>>();
        let rhs_create = select(&select_lhs_cols, rhs_args, &select_rhs_cols, &select_lhs_cols, &select_lhs_cols, &select_column_operators, &select_cast_operators, &select_cast_datatypes, &select_cast_templates, device)?;
        filter(&[operator_column], &[rhs_create], &["create"], &[DataComparatorOperator::Like], &DataComparatorPredicate::All, device)?
    };
...
pub use todo::Todo"#,
        ...
        let result = apply_patch(&[repo_batch.clone()], &[patch_batch.clone()], "code", &["patch", "operation"], "repo_path", "patch_path", &device)?;
        ...
        assert_eq!(test, ["/home/sandbox/Cargo.toml", "/home/sandbox/src/extras/mod.rs", "/home/sandbox/src/extras/todo.rs", "/home/sandbox/src/lib.rs", "/home/sandbox/src/extras/other.rs"]);
        let test = result_table.get_column_as_vec_nonprimitive::<String>("code")?;
        assert_eq!(test, ["[package]\nname = \"phymes_rs\"\nversion = \"0.1.0\"\nedition = \"2024\"\n[dependencies]\nanyhow = { version = \"1\", default-features = false }", "pub mod other;\nmod todo;\npub use todo::Todo", "pub struct Todo {}", "pub mod extra;", "pub struct Other {}"]);

        // --- PK = UInt32 ---
        // Patch the repository
        let result = apply_patch(&[repo_batch], &[patch_batch], "code", &["patch", "operation"], "repo_pk", "patch_pk", &device)?;
        ...
        let test = result_table.get_column_as_vec_nonprimitive::<String>("repo_path")?;
        assert_eq!(test, ["/home/sandbox/Cargo.toml", "/home/sandbox/src/lib.rs", "/home/sandbox/src/extras/mod.rs", "/home/sandbox/src/extras/todo.rs", ""]);
        let test = result_table.get_column_as_vec_nonprimitive::<String>("code")?;
        assert_eq!(test, ["[package]\nname = \"phymes_rs\"\nversion = \"0.1.0\"\nedition = \"2024\"\n[dependencies]\nanyhow = { version = \"1\", default-features = false }", "pub mod extra;", "pub mod other;\nmod todo;\npub use todo::Todo", "pub struct Todo {}", "pub struct Other {}"]);
...
pub use todo::Todo"#,
        ...
        let operations = vec![PatchOperator::Update.to_string(), PatchOperator::Create.to_string()];
        let patches = [
            "@@ pub mod extra;\n+pub mod other;\n",
            "+pub struct Other {}\n*** End Patch",
        ].into_iter().map(|s| s.to_string()).collect::<Vec<_>>();
        ...
        let test = result_table.get_column_as_vec_nonprimitive::<String>("repo_path")?;
        assert_eq!(test, ["/home/sandbox/Cargo.toml", "/home/sandbox/src/extras/mod.rs", "/home/sandbox/src/extras/todo.rs", "/home/sandbox/src/lib.rs", "/home/sandbox/src/main.rs", "/home/sandbox/src/extras/other.rs"]);
        let test = result_table.get_column_as_vec_nonprimitive::<String>("code")?;
        assert_eq!(test, ["[package]\nname = \"phymes_rs\"\nversion = \"0.1.0\"\nedition = \"2024\"\n[dependencies]\nanyhow = { version = \"1\", default-features = false }", "pub mod other;\nmod todo;\npub use todo::Todo", "pub struct Todo {}", "pub mod extra;", "use anyhow::Result;\nfn main() -> Result<()> {\n    Ok(())\n}", "pub struct Other {}"]);
        ...
        let test = result_table.get_column_as_vec_nonprimitive::<String>("repo_path")?;
        assert_eq!(test, ["/home/sandbox/Cargo.toml", "/home/sandbox/src/main.rs", "/home/sandbox/src/lib.rs", "/home/sandbox/src/extras/mod.rs", "/home/sandbox/src/extras/todo.rs", ""]);
        let test = result_table.get_column_as_vec_nonprimitive::<String>("code")?;
        assert_eq!(test, ["[package]\nname = \"phymes_rs\"\nversion = \"0.1.0\"\nedition = \"2024\"\n[dependencies]\nanyhow = { version = \"1\", default-features = false }", "use anyhow::Result;\nfn main() -> Result<()> {\n    Ok(())\n}", "pub mod extra;", "pub mod other;\nmod todo;\npub use todo::Todo", "pub struct Todo {}", "pub struct Other {}"]);
...
pub use todo::Todo"#,
        ...
        let operations = vec![PatchOperator::Delete.to_string(), PatchOperator::Create.to_string()];
        let patches = [
            "",
            "+pub struct Other {}\n*** End Patch",
        ].into_iter().map(|s| s.to_string()).collect::<Vec<_>>();
        ...
        assert_eq!(test, ["/home/sandbox/Cargo.toml", "/home/sandbox/src/extras/mod.rs", "/home/sandbox/src/extras/todo.rs", "/home/sandbox/src/lib.rs", "/home/sandbox/src/extras/other.rs"]);
        let test = result_table.get_column_as_vec_nonprimitive::<String>("code")?;
        assert_eq!(test, ["[package]\nname = \"phymes_rs\"\nversion = \"0.1.0\"\nedition = \"2024\"\n[dependencies]\nanyhow = { version = \"1\", default-features = false }", "mod todo;\npub use todo::Todo", "pub struct Todo {}", "pub mod extra;", "pub struct Other {}"]);
        ...
        assert_eq!(test, ["/home/sandbox/Cargo.toml", "/home/sandbox/src/lib.rs", "/home/sandbox/src/extras/mod.rs", "/home/sandbox/src/extras/todo.rs", ""]);
        let test = result_table.get_column_as_vec_nonprimitive::<String>("code")?;
        assert_eq!(test, ["[package]\nname = \"phymes_rs\"\nversion = \"0.1.0\"\nedition = \"2024\"\n[dependencies]\nanyhow = { version = \"1\", default-features = false }", "pub mod extra;", "mod todo;\npub use todo::Todo", "pub struct Todo {}", "pub struct Other {}"]);
...
pub use todo::Todo"#,
        ...
        let operations = vec![PatchOperator::Delete.to_string(), PatchOperator::Update.to_string()];
        let patches = [
            "",
            "@@ pub mod extra;\n+pub mod other;\n",
        ].into_iter().map(|s| s.to_string()).collect::<Vec<_>>();
        ...
        let test = result_table.get_column_as_vec_nonprimitive::<String>("repo_path")?;
        assert_eq!(test, ["/home/sandbox/src/extras/mod.rs", "/home/sandbox/Cargo.toml", "/home/sandbox/src/extras/todo.rs", "/home/sandbox/src/lib.rs"]);
        let test = result_table.get_column_as_vec_nonprimitive::<String>("code")?;
        assert_eq!(test, ["pub mod other;\nmod todo;\npub use todo::Todo", "[package]\nname = \"phymes_rs\"\nversion = \"0.1.0\"\nedition = \"2024\"\n[dependencies]\nanyhow = { version = \"1\", default-features = false }", "pub struct Todo {}", "pub mod extra;"]);
        ...
        let test = result_table.get_column_as_vec_nonprimitive::<String>("repo_path")?;
        assert_eq!(test, ["/home/sandbox/src/extras/mod.rs", "/home/sandbox/Cargo.toml", "/home/sandbox/src/lib.rs", "/home/sandbox/src/extras/todo.rs"]);
        let test = result_table.get_column_as_vec_nonprimitive::<String>("code")?;
        assert_eq!(test, ["pub mod other;\nmod todo;\npub use todo::Todo", "[package]\nname = \"phymes_rs\"\nversion = \"0.1.0\"\nedition = \"2024\"\n[dependencies]\nanyhow = { version = \"1\", default-features = false }", "pub mod extra;", "pub struct Todo {}"]);
