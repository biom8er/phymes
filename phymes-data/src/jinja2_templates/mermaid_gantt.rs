...
///  {{ row.task }}:{%- if row.start %}{{ row.start }}{%- else %}0{%- endif %}, {{ row.end }}
pub static MERMAID_GANTT_TEMPLATE: &'static str = r#"
        gantt
            title "{{ title }}"
...
pub static MERMAID_GANTT_TABLE_EXPRESSION: &'static str = "rows";
...
///     .join("");
/// ```
pub static MERMAID_GANTT_INPUT: &'static str = r#"{
"title": "{{ title }}",
"dateFormat": "{{ dateFormat }}",
"axisFormat": "{{ axisFormat }}"
}"#;
...
mod tests {
    ...
    fn test_mermaid_gantt_html() -> Result<()> {
        ...
        let section_vec = ["Section 1", "Section 1", "Section 1", "Section 2", "Section 2", "Section 2"]
            .into_iter()
            .map(|s| s.to_string())
            .collect::<Vec<_>>();
        ...
        let input_string = TableScript::new_from_template(MERMAID_GANTT_INPUT.to_string()).apply_template(&inputs)?
            .lines()
            .map(|line| line.trim())
            .collect::<Vec<&str>>()
            .join("");
        ...
        let script_string = TableScript::new_from_template(template).apply_template(&gantt_template_inputs)?;
