...
impl BertEmbeddings {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        //)?;
        let word_embeddings =
            Embedding::new(config.vocab_size, config.hidden_size, vb.pp("token_embd"))?;
...
impl BertSelfAttention {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let query = linear(hidden_size, all_head_size, vb.pp("attn_q"))?;
        let value = linear(hidden_size, all_head_size, vb.pp("attn_k"))?;
        let key = linear(hidden_size, all_head_size, vb.pp("attn_v"))?;
        Ok(Self {
            query,
            key,
            value,
...
impl BertSelfOutput {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let dense = linear(config.hidden_size, config.hidden_size, vb.pp("attn_output"))?;
        let layer_norm = layer_norm(
            config.hidden_size,
            config.layer_norm_eps,
...
impl BertAttention {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let self_attention = BertSelfAttention::load(vb.clone(), config)?;
        let self_output = BertSelfOutput::load(vb.clone(), config)?;
        Ok(Self {
            self_attention,
            self_output,
...
impl BertLayer {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let attention = BertAttention::load(vb.clone(), config)?; //?
        let intermediate = BertIntermediate::load(vb.clone(), config)?; //?
        let output = BertOutput::load(vb.clone(), config)?; //?
        Ok(Self {
            attention,
            intermediate,
            output,
...
impl BertEncoder {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let layers = (0..config.num_hidden_layers)
            .map(|index| BertLayer::load(vb.pp(format!("blk.{index}")), config))
...
impl BertModel {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let (embeddings, encoder) = match (
            BertEmbeddings::load(vb.clone(), config),
            BertEncoder::load(vb.clone(), config),
        ) {
