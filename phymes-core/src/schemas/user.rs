...
pub fn create_user_fields() -> Fields {
    ...
    let session_contexts = Field::new("session_contexts", list_data_type.clone(), false);
    let session_context_builders = Field::new("session_context_builders", list_data_type, false);
    Fields::from(vec![
        email,
        first_name,
        last_name,
...
pub struct UserSubject {
    ...
    pub session_context_builders: Vec<String>,
}
...
pub fn create_user_batch(
    ...
    session_context_builders: Vec<Vec<String>>,
) -> Result<RecordBatch> {
    ...
    let value_builder = StringBuilder::new();
    let mut list_builder = ListBuilder::new(value_builder).with_field(Field::new_list_field(DataType::Utf8, false));
    for values in session_context_builders.into_iter() {
        for value in values.into_iter() {
            list_builder.values().append_value(&value);
        }
        list_builder.append(true);
    }
    let session_context_builders: ArrayRef = Arc::new(list_builder.finish());
    let batch = RecordBatch::try_from_iter(vec![
        ...
        ("session_context_builders", session_context_builders),
    ])?;
    Ok(batch)
}

pub trait UserBuilderTraitExt: Sized {
    fn with_user(self, email: Option<&str>, first_name: Option<&str>, last_name: Option<&str>, password_hash: Option<&str>, session_contexts: Option<Vec<String>>, session_context_builders: Option<Vec<String>>) -> Result<Self>;
}

impl UserBuilderTraitExt for TableBuilder {
    fn with_user(mut self, email: Option<&str>, first_name: Option<&str>, last_name: Option<&str>, password_hash: Option<&str>, session_contexts: Option<Vec<String>>, session_context_builders: Option<Vec<String>>) -> Result<Self> {
        // Handle the metadata
        ...
        let session_context_builders = session_context_builders.unwrap_or_default();
