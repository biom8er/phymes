use crate::{
    BuildableTrait, BuilderTrait, IndexType, MappableTrait, Subject, SubjectBuilderTrait,
    SubjectConstraintType, SubjectPlan, SubjectSequenceType,
};
...
impl BuilderTrait for SubjectPlanBuilder {
    ...
    {
        let t = Self::T {
            subject: self.subject.ok_or(anyhow!(
                "Please define the subject before trying to build the subject plan!"
            ))?,
...
impl SubjectPlanBuilderTrait for SubjectPlanBuilder {
    ...
    fn get_name(&self) -> Result<String> {
        let name = self
            ...
            .as_ref()
            .ok_or(anyhow!(
                "Please define the subject before trying to get the subject name!"
            ))?
            .get_name()
            .to_string();
        Ok(name)
