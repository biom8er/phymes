...
pub struct DiffWorkspaceNetworkBuilderStaticWSubject {
    pub inner: DynamicTaskNetworkBuilder,
}

impl Default for DiffWorkspaceNetworkBuilderStaticWSubject {
    fn default() -> Self {
        // Initialize the task data
        let network_name = "diff";
        let subject_name_lhs = "other_workspace_s";

        // Processor subject
        let config = DataConfig {
            lhs_name: Some(AvailableSubjects::Workspace.to_string()),
            rhs_name: Some(subject_name_lhs.to_string()),
            lhs_values: Some(vec!["content".to_string()]),
            rhs_values: Some(vec!["content".to_string()]),
...
pub struct DiffWorkspaceNetworkBuilderDynamicWOSubject {
    pub inner: DynamicTaskNetworkBuilder,
}

impl Default for DiffWorkspaceNetworkBuilderDynamicWOSubject {
    fn default() -> Self {
        // Initialize the task data
        let network_name = "diff";
        let subject_name_lhs = "other_workspace_s";
        ...
        let (network, network_messages) = diff_workspace_network
            ...
            .with_runtime_env(
                RuntimeEnvBuilder::default()
                    .with_name(
                        &DynamicTaskNetworkNames::Task(&diff_workspace_network.inner.network_name)
                            .to_string(),
                    )
                    .build_arc()?,
            )
...
pub use todo::Todo"#,
            ...
            let _ = message_map.insert(
                ...
                IPCMessage::get_builder()
                    ...
                    .with_publisher(&diff_workspace_network.inner.network_name)
            ...
            let diff_config_table = SubjectBuilder::new()
                ...
                .build()?;
            let _ = message_map.insert(
                diff_config_table.get_name().to_string(),
                IPCMessage::get_builder()
                    .with_name(diff_config_table.get_name())
                    .with_publisher(&diff_workspace_network.inner.network_name)
                    .with_subject(diff_config_table.get_name())
                    .with_update(&Publication::Replace {
                        subject_name: diff_config_table.get_name().to_string(),
                    })
                    .with_message(diff_config_table.to_ipc_stream()?)
                    .build()?,
            );
        }
