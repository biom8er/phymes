...
mod tests {
    ...
    fn test_task_plan_individualize() {
        ...
        .unwrap();
        let test_task_plan = TaskPlan {
            task_name: test_task.name,
            processor_names: test_task
                .processor
                .into_iter()
        ...
        assert_eq!(
            individuals.first().unwrap().processor_names.last().unwrap(),
            "test_processor_1"
        );
        assert_eq!(individuals.get(1).unwrap().task_name, "test_task_1");
        assert_eq!(
            individuals.get(1).unwrap().processor_names.last().unwrap(),
            "test_processor_2"
        );
        assert_eq!(individuals.get(2).unwrap().task_name, "test_task_2");
        assert_eq!(
            individuals.get(2).unwrap().processor_names.last().unwrap(),
            "test_processor_3"
        );
    }
    ...
    fn test_task_plan_extend() {
        ...
        assert_eq!(
            test_task_plan.processor_names.first().unwrap(),
            "test_processor_1"
        );
        assert_eq!(
            test_task_plan.processor_names.last().unwrap(),
            "test_processor_2"
        );
    }
}
