...
pub struct DownloadPDFSession<'a> {
    /// Session
    pub session_context_name: &'a str,
}

impl<'a> Default for DownloadPDFSession<'a> {
    fn default() -> Self {
        Self {
            session_context_name: "download_pdf_session",
        }
    }
}

impl<'a> DownloadPDFSession<'a> {
	...
	subgraph download_pdf_t
		...
		download_pdf_s-subject-->|FullTable|chunk_documents_p-subscribe
	...
	download_pdf_r-rt@{shape: subproc, label: download_pdf_r}
	download_pdf_r-rt-->download_pdf_t
	UserPdf-subject@{shape: doc, label: UserPdf}
	download_pdf_p-processor@{shape: rect, label: ExtractPDF}
	download_pdf_p-publish@{shape: fork}
	download_pdf_p-subscribe@{shape: diamond, label: All}
	download_pdf_s-subject@{shape: doc, label: chunk_documents_task_1}
...
mod tests {
    ...
    async fn test_download_pdf_session() -> Result<()> {
        // Initialize the session
        let download_pdf_session = DownloadPDFSession::default();
        let session_ctx = SessionContextBuilder::from_mermaid_flowchart(
            download_pdf_session.as_mermaid_flowchart(),
            false,
        )?
        .with_state_from_mermaid_erdiagram(download_pdf_session.as_mermaid_erdiagram(), false, true)?
        .with_name(download_pdf_session.session_context_name)
        ...
        let blob_message = IPCMessage::get_builder()
            ...
            .with_publisher(download_pdf_session.session_context_name)
            .make_name()?
            .build()?;
