...
use candle_nn::{Embedding, LayerNorm, VarBuilder, layer_norm};
use serde::Deserialize;
use tei_backend_core::{Batch, Model, ModelType, Pool};
...
impl NomicBertEmbeddings {
    ...
    pub fn forward(&self, input_ids: &Tensor, token_type_ids: &Tensor) -> Result<Tensor> {
        ...
        let _token_type_embeddings = self.token_type_embeddings.forward(token_type_ids)?;

        let embeddings = self.layer_norm.forward(&input_embeddings)?;
...
impl NomicBertGatedMLP {
    pub fn load(vb: VarBuilder, config: &NomicConfig) -> Result<Self> {
        ...
        let gate_up_proj = NomicLinear::new(
            gate_up_proj_weight,
            None,
        ...
        let down_proj = NomicLinear::new(down_proj_weight, None, None);
...
struct NomicAttention {
    qkv_linear: NomicLinear,
    out_proj: NomicLinear,
...
impl NomicAttention {
    pub fn load(vb: VarBuilder, config: &NomicConfig) -> Result<Self> {
        ...
        let qkv_linear = NomicLinear::new(qkv_weight, None, None);

        let out_proj_weight = vb
            .pp("out_proj")
            .get((hidden_size, hidden_size), "weight")?;
        let out_proj = NomicLinear::new(out_proj_weight, None, None);
...
impl NomicBertBlock {
    ...
    pub fn forward(
        ...
        let _attn_output = self
            .attention
            .forward(hidden_states, attention_bias, cos, sin)?;
        let hidden_states = self.post_attention_layer_norm.forward(hidden_states)?;

        let _mlp_out = self.mlp.forward(&hidden_states)?;
